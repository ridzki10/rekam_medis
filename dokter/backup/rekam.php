...
include "koneksi.php";
if (isset($_GET['no_rm'])){
$d = "SELECT no_rm, kd_tindakan, kd_obat, kd_user, no_pasien, diagnosa, resep, keluhan, tgl_pemeriksaan, tgl_kesembuhan, ket FROM rekam_medis WHERE no_rm ='$_GET[no_rm]'";
...
if (isset($_POST['txtcari'])){
 ...
 //4. Batasi dengan limit
  	$a = ("SELECT A.no_rm, B.nm_tindakan, B.ket, C.nm_obat, C.jml_obat, C.ukuran, C.harga, D.username, E.nm_pasien, E.jenis_kelamin, E.alamat, A.diagnosa, A.resep, A.keluhan, A.tgl_pemeriksaan, A.tgl_kesembuhan , A.ket FROM rekam_medis AS A INNER JOIN tindakan AS B ON (A.kd_tindakan=B.kd_tindakan) INNER JOIN obat AS C ON (A.kd_obat=C.kd_obat) INNER JOIN login AS D ON (A.kd_user=D.kd_user) INNER JOIN pasien AS E ON (A.no_pasien=E.no_pasien) WHERE E.nm_pasien LIKE '%$katakunci%'LIMIT $start,$hal");
